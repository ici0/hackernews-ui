...
SECRET_PATH=$(grep -oE "SYNC_ENDPOINT = 'http://[^']*(/hn-[0-9a-f]+)'" "$HTML" | grep -oE '/hn-[0-9a-f]+' || true)
...
sed "s|__SECRET_PATH__|$SECRET_PATH|" k8s.yaml | kubectl apply -f -
...
ENDPOINT="http://$EXTERNAL_IP$SECRET_PATH"
...
echo "Smoke test:"
curl -sf --max-time 10 "$ENDPOINT" && echo " <- GET OK"
